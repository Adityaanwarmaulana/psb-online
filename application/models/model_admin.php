...
class Model_admin extends CI_Model
{
	...
	function psb_status($option, $date)
	{
		switch ($option) {
			case 'tutup':
				$data = array(
	...
	function set_announce($act = null, $id = '')
	{
		switch ($act) {
			case 'lulus':
				...
				return $this->db->update('tbl_santri', $data, array('no_pendaftaran' => "$id"));
				break;
	...
	function edit_santri($menu = '', $data = '')
	{
		switch ($menu) {
			case 'update':
				$old_user = $data['old_user'];
				$data = array(
					...
					'paud'					=> $data['paud'],
					'tk'					=> $data['tk'],
					'no_hp_santri'			=> $data['no_hp_santri'],
				...
				return $this->db->update('tbl_santri', $data, array('no_pendaftaran' => $old_user));
				break;
	...
	function verifikasi($menu = '', $thn = '', $status = '')
	{
		switch ($menu) {
			case 'santri':
				$res = $this->db->like('tgl_santri', "$thn", 'after')->order_by('id_santri', 'DESC')->get('tbl_santri');
				return (object) array(
					'bar'	=> $res->row(),
					'sum'	=> $res->num_rows(),
					'ori'	=> $res
			...
			case 'acc':
				switch ($thn) {
					case 'total':
						return $this->db->like('tgl_santri', "$thn", 'after')->get("tbl_santri")->num_rows();
						break;

					case 'diverifikasi':
						return $this->db->like('tgl_santri', "$thn", 'after')->where('status_verifikasi', '1')->get("tbl_santri")->num_rows();
						break;

					case 'diterima':
						return $this->db->like('tgl_santri', "$thn", 'after')->where('status_pendaftaran', 'lulus')->get("tbl_santri")->num_rows();
						break;

					case 'tidak diterima':
						return $this->db->like('tgl_santri', "$thn", 'after')->where('status_pendaftaran', 'tidak lulus')->get("tbl_santri")->num_rows();
						break;
	...
	function update($menu = '', $data = '')
	{
		switch ($menu) {
			...
			case 'change-stu-verif':
				...
				$this->db->update('tbl_santri', $param, array('no_pendaftaran' => $data['id']));
				break;
