...
class Model_web extends CI_Model
{
		...
		switch ($menu) {
			case 'daftar':
				...
				$data = array(
					...
					'paud'				=> $data->post('paud'),
					'tk'				=> $data->post('tk'),
					'no_hp_santri'		=> $data->post('no_hp_santri'),
					'jenis_tinggal'		=> $data->post('jenis_tinggal'),
					'alamat_santri'		=> $data->post('alamat_santri'),
					'desa'				=> $data->post('desa'),
					'kec'				=> $data->post('kec'),
					'kab'				=> $data->post('kab'),
					'prov'				=> $data->post('prov'),
					...
					'tgl_santri'			=> date('Y-m-d H:i:s')
				);
				return $this->db->insert('tbl_santri', $data);
				break;

			case 'id_baru':
				$no_max = $this->db->select_max('no_pendaftaran', 'kode')->get('tbl_santri')->row();
				$no_max = $no_max->kode;
	...
	function auth($menu = '', $data = '')
	{
		switch ($menu) {
			case 'cek-masuk':
				$query = $this->db->where("no_pendaftaran", $data['username'])->where("password", $data['password'])->get('tbl_santri');
				return array(
					'res'	=> $query->row(),
					'sum'	=> $query->num_rows()
				);
