...
class Santri extends CI_Controller {
  ...
  public function export(){
    ...
    $excel->setActiveSheetIndex(0)->setCellValue('A1', "DAFTAR PENERIMAAN SANTRI BARU"); // Set kolom A1 dengan tulisan "DATA SANTRI"
    ...
    // Panggil function view yang ada di SantriModel untuk menampilkan semua data santrinya
    $santri = $this->SantriModel->view();

    $no = 1; // Untuk penomoran tabel, di awal set dengan 1
    $numrow = 4; // Set baris pertama untuk isi tabel adalah baris ke 4
    foreach($santri as $data){ // Lakukan looping pada variabel santri
      ...
      $excel->setActiveSheetIndex(0)->setCellValueExplicit('P'.$numrow, $data->no_hp_santri, PHPExcel_Cell_DataType::TYPE_STRING);
      $excel->setActiveSheetIndex(0)->setCellValue('Q'.$numrow, $data->jenis_tinggal);
      $excel->setActiveSheetIndex(0)->setCellValue('R'.$numrow, $data->alamat_santri);
    ...
    $excel->getActiveSheet(0)->setTitle("Data Santri");
    $excel->setActiveSheetIndex(0);

    // Proses file excel
    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    header('Content-Disposition: attachment; filename="Export Data Santri.xlsx"'); // Set nama file excel nya
