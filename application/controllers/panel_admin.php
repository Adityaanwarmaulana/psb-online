...
class Panel_admin extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($this->session->userdata('id_admin') == NULL) {
			...
			$data = array(
				...
				'web_psb'	=> $this->admin->base('status-psb'),
				'judul_web'	=> "HOME",
				'v_thn'		=> date('Y'),
			...
			if (isset($_POST['btnnonaktif'])) {
				$acts = $this->admin->psb_status('tutup', date('Y-m-d H:i:s'));
				redirect('panel_admin');
			}

			if (isset($_POST['btnaktif'])) {
				$acts = $this->admin->psb_status('buka', date('Y-m-d H:i:s'));
				redirect('panel_admin');
			}
		}
	}
	...
	public function ubah_santri($aksi = '', $id = '')
	{
		$sess = $this->session->userdata('id_admin');
		...
		if ($sess == NULL) {
			...
			$data = array(
				...
				'v_santri'	=> $this->admin->verifikasi('santri', $thn)->ori,
				'v_thn'		=> $thn
			);
			...
			if (isset($_POST['btnupdate'])) {
				...
				$data = array(
					'old_user'				=> $this->session->userdata('id_santri'),
					...
					'no_hp_santri'			=> $this->session->userdata('no_hp_santri'),
					'jenis_tinggal'			=> $this->session->userdata('jenis_tinggal'),
					'alamat_santri'			=> $this->session->userdata('alamat_santri'),
				...
				$acts = $this->admin->edit_santri('update', $data);
				$this->session->has_userdata('id_santri');
				$this->session->set_userdata('id_santri', $data['no_pendaftaran']);
	...
	public function verifikasi($aksi = '', $id = '')
	{
		...
		if ($sess == NULL) {
			...
			switch ($aksi) {
				case 'cek':
					$cek_status = $this->santri->base_biodata($id);
					$data = array(
						'id'				=> $id,
	...
	public function verifikasi_cetak($id = '')
	{
		...
		$data = array(
			'user'			=> $this->santri->base_biodata($id),
			'judul_web'		=> "CETAK_VERIFIKASI_" . ucwords($this->santri->base_biodata($id)->no_pendaftaran),
			'thn_psb'		=> date('Y', strtotime($this->santri->base_biodata($id)->tgl_santri)),
			'v_materi'		=> $this->admin->verifikasi('materi')
		);
	...
	public function export($aksi = '', $id = '')
	{
		...
		if ($sess == NULL) {
			...
			$data = array(
				'user'		=> $this->admin->base('bio', $sess),
				'judul_web'	=> "EXPORT KE EXCEL HASIL FORMULIR PENDAFTARAN SANTRI (BIODATA SANTRI)",
				'v_santri'	=> $this->admin->verifikasi('santri', $thn)->ori,
				'v_thn'		=> $thn
			);
			var_dump($data);
	...
	public function set_pengumuman($aksi = '', $id = '')
	{
		...
		if ($sess == NULL) {
			...
			$data = array(
				'user'		=> $this->admin->base('bio', $sess),
				'judul_web'	=> "KELULUSAN",
				'v_santri'	=> $this->admin->verifikasi('santri', $id)->ori,
				'v_thn'		=> $thn
			);
	...
	{
		$this->Model_admin->delete($id_santri);
